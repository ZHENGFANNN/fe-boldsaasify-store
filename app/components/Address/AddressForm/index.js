...
// 结账页游客地址表单：不入库，仅通过 ref.onSubmit() 返回结构化字段给父级下单使用；
// 兼容登录用户「新增地址」的字段口径，回填源为 localStorage.address_form（游客上次填写）。
//
// props:
//  - LANG: 语言包
//  - apiParse: 粘贴文本 → 结构化地址(可选)
//  - onStateChange?: 州变化回调(用于结账预览带 shipping_address.state 复算)
//  - ref.onSubmit(): 返回表单值(缺字段/校验不通过时返回 null)
function AddressForm({ LANG, apiParse, onStateChange }, ref) {
