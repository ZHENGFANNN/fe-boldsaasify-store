...
function getExcludePath() {
  return ['/user/forget', '/user/account', '/cart', '/order']
}

// 获取导航栏路径
function getNavPath() {
  const pathList = []
  const navList = ['/nav/product_categories', '/nav/product_info', '/nav/where_buy', '/nav/support', '/nav/about_us']
...
function getProductPath() {
  ...
  LANGUAGES('list').forEach((item) => {
    productList[item.value].forEach((product) => {
      pathList.push(`/${item.value}/product/${product.sort_key}/${product.key}`, `/${item.value}/store/product/${product.sort_key}/${product.key}`)
    })
  })
  return pathList
}
...
const getAllPages = (dir = '') => {
  const pagesDir = join(process.cwd(), 'src/pages', dir)
  ...
  files.forEach((file) => {
    if (fs.statSync(join(pagesDir, file)).isDirectory()) {
      ...
      if (file !== '_app.js' && file !== '_document.js' && file.endsWith('.jsx')) {
...
async function getSitMap(times = 1) {
  ...
  try {
    ...
    pages.forEach((page) => {
      ...
      if (page.includes('[locale]')) {
        for (const locale of locales) {
          const url = page.replace(`[locale]`, locale)
          // 排查特殊页面
          ...
          // ***排除产品页、导航页、
          if (!url.includes('/nav/[type]') && !url.includes('/product/[sortKey]/[productKey]') && !isExcludeUrl) {
    ...
    const sitemap = (await streamToPromise(Readable.from(allPages).pipe(stream))).toString()
    const filePath = join(process.cwd(), 'public', 'sitemap.xml')
    fs.writeFileSync(filePath, sitemap)
    console.log(`${chalk.green('【SITEMAP获取时长】')} ${new Date().getTime() - startTime}ms`)
  } catch (err) {
    error = true
