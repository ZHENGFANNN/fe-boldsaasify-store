...
async function getComboList({ area, productInfo }) {
  const { setProductCurCombo } = useProductStore.getState();
  ...
  setProductCurCombo(list[0] ?? { areaInfo: [] });
  return list;
}

// 获取类型
async function getTypeList({ productInfo, LANG }) {
  const { setProductShowType } = useProductStore.getState();
  const list = [];
  ...
  setProductShowType(list[0].type);
  return list;
}
